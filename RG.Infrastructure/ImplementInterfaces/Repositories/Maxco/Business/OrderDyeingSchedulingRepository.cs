...
{
    public class OrderDyeingSchedulingRepository:GenericRepository<OrderDyeingScheduling>, IOrderDyeingSchedulingRepository
    {
        private readonly MaxcoDBContext _dbCon;

        public OrderDyeingSchedulingRepository(MaxcoDBContext dbCon):base(dbCon)
        {
            _dbCon = dbCon;
        }
        ...
        public async Task<List<OrderDyeingSchedulingRM>> GetListDyeingFinishFabric(int OrderId, int GSM, int FabricQualityID, string FabricComposition, string PantoneNo, decimal FinishedWidth, CancellationToken cancellationToken)
        {
            var dataQuery = from os in _dbCon.OrderScheduling
                       join df in _dbCon.OrderDyeingScheduling on new { schid = os.ScheduleID, act = true, del = false } equals new { schid = df.ScheduleID, act = df.IsActive, del = df.IsRemoved }
                       ...
                       select new OrderDyeingSchedulingRM()
                       {
                           DyeingScheduleID = df.DyeingScheduleID,
            ...
            if (data == null)
            {
                data = new List<OrderDyeingSchedulingRM>();
            }
            return   data;
        }
    }
